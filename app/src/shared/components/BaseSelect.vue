...
onClickOutside(selectContainer, closeDropdown);
...
<template>
  <div ref="selectContainer" class="relative w-fit font-inherit text-inherit">
    <button
      v-bind="$attrs"
      :id="buttonId"
      type="button"
      ...
      class="flex items-center justify-between gap-4 appearance-none border border-solid border-(--border) min-w-[8em] max-w-[16em] min-h-[2em] px-2 py-[0.2rem] text-(--text-default-color) transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <li
          ...
          :class="[
            modelValue === option.value
              ? 'bg-(--accent) text-(--text-on-accent-color)'
              : '',
            highlightedIndex === index && modelValue !== option.value
              ? 'border border-solid border-(--border) bg-(--option-hover)'
              : '',
          ]"
